...
export default function UpdatePlate({categories}) {
  const [error, setError] = useState(null)
  const { plates, updatePlate } = useContext(FetchPlatesContext) // implementar updatePlate en el context
        ...
        <div className="w-2/5 h-fit p-2 flex flex-col gap-8">
          {filteredPlates.map((plate) => {
            return (
              ...
              >
                <Link to={`/plato/actualizar/${plate.id}`}>
                <img
                  src={UpdateSrc}
                  alt="delete"
                  className="absolute -top-5 -right-4 w-8 h-8 cursor-pointer z-20"
                />
                </Link>
